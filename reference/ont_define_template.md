...
Creates a template that serves as a base definition for related
concepts. Templates can contain parameter placeholders (e.g.,
`{{age_threshold}}`) that variants can customize when inheriting.
...
- base_sql_expr:
  ...
  Character. SQL expression with optional `{{placeholder}}` parameters
  that variants can customize.

- parameters:

  Named list. Parameter definitions with default values. Each parameter
  can be a simple default value or a list with `default`, `type`, and
  `description` fields.
