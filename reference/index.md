# Package index

## All functions

- [`analysis`](https://cathalbyrnegit.github.io/ontologyR/reference/analysis.md)
  : Observations and Analysis
- [`audit`](https://cathalbyrnegit.github.io/ontologyR/reference/audit.md)
  : Audit Functions
- [`concepts`](https://cathalbyrnegit.github.io/ontologyR/reference/concepts.md)
  : Concept Definition and Evaluation
- [`connection`](https://cathalbyrnegit.github.io/ontologyR/reference/connection.md)
  : Connection Management
- [`datasets`](https://cathalbyrnegit.github.io/ontologyR/reference/datasets.md)
  : Dataset Registry and Materialization
- [`drift`](https://cathalbyrnegit.github.io/ontologyR/reference/drift.md)
  : Drift Detection
- [`governance`](https://cathalbyrnegit.github.io/ontologyR/reference/governance.md)
  : Governance Functions
- [`hybrid`](https://cathalbyrnegit.github.io/ontologyR/reference/hybrid.md)
  : Hybrid API Layer
- [`objects`](https://cathalbyrnegit.github.io/ontologyR/reference/objects.md)
  : Object and Link Type Registration
- [`ont_activate_version()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_activate_version.md)
  : Activate a Concept Version
- [`ont_add_cohort_members()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_add_cohort_members.md)
  : Add Members to a Cohort
- [`ont_add_transform_input()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_add_transform_input.md)
  : Add Transform Input
- [`ont_add_version()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_add_version.md)
  : Add a Concept Version
- [`ont_approve_request()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_approve_request.md)
  : Approve Request
- [`ont_audit_summary()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_audit_summary.md)
  : Compute Audit Summary Statistics
- [`ont_check_all_gates()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_check_all_gates.md)
  : Check All Gates
- [`ont_check_drift()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_check_drift.md)
  : Check for Drift
- [`ont_check_gate()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_check_gate.md)
  : Check Gate
- [`ont_check_permission()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_check_permission.md)
  : Check User Permission
- [`ont_check_policy()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_check_policy.md)
  : Check Governance Policies
- [`ont_cohort_compare()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_cohort_compare.md)
  : Cohort Comparison
- [`ont_compare_runs()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_compare_runs.md)
  : Compare Runs
- [`ont_compare_versions()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_compare_versions.md)
  : Compare Concept Versions
- [`ont_connect()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_connect.md)
  : Connect to an Ontology Database
- [`ont_create_gate()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_create_gate.md)
  : Create Governance Gate
- [`ont_create_role()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_create_role.md)
  : Create Custom Role
- [`ont_define_cohort()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_define_cohort.md)
  : Define a Cohort
- [`ont_define_concept()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_define_concept.md)
  : Define a New Concept
- [`ont_define_transform()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_define_transform.md)
  : Define a Transform
- [`ont_deprecate_version()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_deprecate_version.md)
  : Deprecate a Concept Version
- [`ont_deprecation_impact()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_deprecation_impact.md)
  : Analyze Impact of Deprecation
- [`ont_detect_drift()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_detect_drift.md)
  : Run Drift Detection with Alerting
- [`ont_disconnect()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_disconnect.md)
  : Disconnect from Ontology Database
- [`ont_drift_status()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_drift_status.md)
  : Get Drift Status Summary
- [`ont_evaluate()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_evaluate.md)
  : Evaluate a Concept
- [`ont_execute_transform()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_execute_transform.md)
  : Execute Transform
- [`ont_export_json()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_export_json.md)
  : Export Ontology to JSON
- [`ont_get_active_version()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_active_version.md)
  : Get Active Version
- [`ont_get_audits()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_audits.md)
  : Get Audit History
- [`ont_get_cohort_members()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_cohort_members.md)
  : Get Cohort Members
- [`ont_get_dataset()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_dataset.md)
  : Get Dataset Metadata
- [`ont_get_dataset_runs()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_dataset_runs.md)
  : Get Run History for Dataset
- [`ont_get_downstream()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_downstream.md)
  : Get Downstream Lineage
- [`ont_get_gate()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_gate.md)
  : Get Gate Details
- [`ont_get_gate_history()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_gate_history.md)
  : Get Gate Check History
- [`ont_get_governance_log()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_governance_log.md)
  : Get Governance Log
- [`ont_get_impact()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_impact.md)
  : Get Downstream Impact
- [`ont_get_lineage_graph()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_lineage_graph.md)
  : Get Full Lineage Graph
- [`ont_get_observation_details()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_observation_details.md)
  : Get Observation Details
- [`ont_get_provenance()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_provenance.md)
  : Get Dataset Provenance
- [`ont_get_role()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_role.md)
  : Get Role Details
- [`ont_get_run()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_run.md)
  : Get Run Details
- [`ont_get_transform()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_transform.md)
  : Get Transform Details
- [`ont_get_transform_dag()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_transform_dag.md)
  : Get Transform DAG
- [`ont_get_transform_inputs()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_transform_inputs.md)
  : Get Transform Inputs
- [`ont_get_upstream()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_upstream.md)
  : Get Upstream Lineage
- [`ont_get_user_roles()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_get_user_roles.md)
  : Get User Roles
- [`ont_governance_report()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_governance_report.md)
  : Generate Governance Report
- [`ont_grant_role()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_grant_role.md)
  : Grant Role to User
- [`ont_import_json()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_import_json.md)
  : Import Ontology from JSON
- [`ont_list_analyses()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_analyses.md)
  : List Analysis Runs
- [`ont_list_cohorts()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_cohorts.md)
  : List Cohorts
- [`ont_list_concepts()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_concepts.md)
  : List Concepts
- [`ont_list_dashboards()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_dashboards.md)
  : List Dashboard Dependencies
- [`ont_list_datasets()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_datasets.md)
  : List Datasets
- [`ont_list_drift_events()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_drift_events.md)
  : List Drift Events
- [`ont_list_gates()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_gates.md)
  : List Governance Gates
- [`ont_list_links()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_links.md)
  : List Registered Link Types
- [`ont_list_objects()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_objects.md)
  : List Registered Object Types
- [`ont_list_observations()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_observations.md)
  : List Observations
- [`ont_list_pending_approvals()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_pending_approvals.md)
  : List Pending Approvals
- [`ont_list_roles()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_roles.md)
  : List Available Roles
- [`ont_list_runs()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_runs.md)
  : List Runs
- [`ont_list_transforms()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_transforms.md)
  : List Transforms
- [`ont_list_versions()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_list_versions.md)
  : List Concept Versions
- [`ont_log_governance()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_log_governance.md)
  : Log a Governance Action
- [`ont_materialize()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_materialize.md)
  : Materialize a Concept
- [`ont_observe()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_observe.md)
  : Record a Concept Observation
- [`ont_override_gate()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_override_gate.md)
  : Override Gate Check
- [`ont_query_linked()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_query_linked.md)
  : Query Linked Objects
- [`ont_query_objects()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_query_objects.md)
  : Query Objects by Primary Key
- [`ont_record_audit()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_record_audit.md)
  : Record an Audit Judgment
- [`ont_record_audits()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_record_audits.md)
  : Record Multiple Audit Judgments
- [`ont_register_dashboard()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_register_dashboard.md)
  : Register Dashboard Dependency
- [`ont_register_dataset()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_register_dataset.md)
  : Register a Dataset
- [`ont_register_link()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_register_link.md)
  : Register a Link Type
- [`ont_register_object()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_register_object.md)
  : Register an Object Type
- [`ont_reject_request()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_reject_request.md)
  : Reject Request
- [`ont_remove_transform_input()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_remove_transform_input.md)
  : Remove Transform Input
- [`ont_request_approval()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_request_approval.md)
  : Request Approval
- [`ont_require_permission()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_require_permission.md)
  : Require Permission
- [`ont_resolve_drift()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_resolve_drift.md)
  : Resolve a Drift Event
- [`ont_revoke_role()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_revoke_role.md)
  : Revoke Role from User
- [`ont_rolling_disagreement()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_rolling_disagreement.md)
  : Compute Rolling Disagreement Rate
- [`ont_sample_for_audit()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_sample_for_audit.md)
  : Sample Objects for Audit
- [`ont_sql()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_sql.md)
  : Execute Raw SQL
- [`ont_status()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_status.md)
  : Get Connection Status
- [`ont_toggle_gate()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_toggle_gate.md)
  : Enable/Disable Gate
- [`ont_trend_analysis()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_trend_analysis.md)
  : Trend Analysis
- [`ont_update_transform()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_update_transform.md)
  : Update Transform Code
- [`ont_validate_dag()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_validate_dag.md)
  : Validate Transform DAG
- [`ont_validate_sql()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_validate_sql.md)
  : Validate SQL Expression
- [`ont_version_compare()`](https://cathalbyrnegit.github.io/ontologyR/reference/ont_version_compare.md)
  : Version Comparison
- [`ontology()`](https://cathalbyrnegit.github.io/ontologyR/reference/ontology.md)
  : Create an Ontology Object
- [`rbac`](https://cathalbyrnegit.github.io/ontologyR/reference/rbac.md)
  : RBAC-lite and Governance Gates
- [`transforms`](https://cathalbyrnegit.github.io/ontologyR/reference/transforms.md)
  : Transform Management and Execution
- [`utils`](https://cathalbyrnegit.github.io/ontologyR/reference/utils.md)
  : Utility Functions
